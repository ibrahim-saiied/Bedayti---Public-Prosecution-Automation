...
class App(tk.Tk):
    ...
    def start_from_open_browser(self):
        ...
        if found_requests:
            ...
            if self.is_inquiry_service():
                self.run_inquiry_after_login_placeholder()
                return
    ...
    def run_inquiry_after_login_placeholder(self):
        """
        Placeholder مقصود لخدمة الاستعلام.
        اترك التنفيذ فارغًا الآن لحين تحديد متطلبات الاستعلام.
        """
        selected = self.get_selected_service()
        self.status_var.set(f"تم فتح خدمة: {selected} (قيد الإعداد)")
        messagebox.showinfo(
            "قيد التطوير",
            "خدمة الاستعلام عن حالة الطلب غير مفعلة بعد.\n"
            "تم ترك هذا المسار فارغًا عمدًا ليتم تنفيذه لاحقًا."
        )
    ...
    def start(self):
        try:
            self.load_input_data(include_cases=self.is_automation_service())
    ...
    def save_single_request_number_to_excel(self, case_idx, req_no):
        path = self.excel_path.get()
        if not path or not os.path.isfile(path):
            self.err("مسار ملف Excel غير موجود لحفظ رقم الطلب.")
        ...
        try:
            ...
            ws.cell(row=case_idx + 2, column=req_col, value=str(req_no).strip())
            wb.save(path)
        except PermissionError:
            self.err("تعذر حفظ ملف Excel. أغلق الملف من Excel ثم أعد المحاولة.")
    ...
    def next_step(self):
        ...
        try:
            ...
            self.status_var.set("تم تقديم الطلب. جارٍ انتظار رقم الطلب...")
            ...
            if req_no:
                ...
                self.save_single_request_number_to_excel(submitted_idx, req_no)
                self.status_var.set(f"تم حفظ رقم الطلب: {req_no}")
            else:
                self.cases.at[self.cases.index[submitted_idx], "رقم_الطلب"] = ""
                self.status_var.set("تم تقديم الطلب ولكن لم يظهر رقم الطلب بعد.")
            ...
            if self.current_index + 1 >= len(self.case_tabs):
                self.status_var.set("تم تقديم كل الطلبات وحفظ الأرقام في Excel.")
